...
export default function AnniversaryTiesPage() {
  ...
  const [productFilter, setProductFilter] = useState('90th anniversary');
  const [excludeUnitPrice, setExcludeUnitPrice] = useState('');
  ...
  // product should break out of the Commercial & Transformation iframe into a
  ...
  const detailHref = (productName: string) => {
    ...
    return `/agenda/anniversary-ties/${encodeURIComponent(productName)}?${params.toString()}`;
  };
      ...
      <div className={embedded ? 'mx-auto w-full px-2 py-2' : 'mx-auto w-full max-w-6xl px-4 py-10'}>
        <header className={embedded ? 'mb-4' : 'mb-8'}>
          ...
          <p className="mt-1 text-zinc-600">
            ...
            <a
              href="https://www.aldwinians.co.uk/club-stash"
              target="_blank"
          ...
          <div className="flex flex-wrap items-end gap-3 px-6 py-5">
            ...
            <div className="flex-1 min-w-[220px]">
              ...
              <input
                ...
                placeholder="90th anniversary"
                className="w-full rounded-md border border-zinc-300 px-3 py-2 text-sm"
              />
              <p className="mt-1 text-xs text-zinc-500">
                Matches against the Squarespace product name — adjust if the club-stash listing is named differently.
              </p>
            </div>
          ...
          <>
            <section className="mb-6 grid grid-cols-1 gap-4 sm:grid-cols-3">
              <div className="rounded-lg bg-white p-5 shadow-sm ring-1 ring-zinc-200">
                <p className="text-sm text-zinc-500">📦 Ties sold</p>
              ...
              <div className="rounded-lg bg-white p-5 shadow-sm ring-1 ring-zinc-200">
                <p className="text-sm text-zinc-500">🔁 Repeat buyers</p>
                <p className="mt-1 text-3xl font-bold text-zinc-900">{repeatBuyers.length}</p>
                <p className="mt-1 text-xs text-zinc-400">bought more than one variant/product in this range</p>
              </div>
            </section>
            ...
            <section className="mb-6 rounded-lg bg-white shadow-sm ring-1 ring-zinc-200">
              <div className="border-b border-zinc-200 px-6 py-4">
                ...
                <p className="mt-0.5 text-sm text-zinc-500">Who ordered what, most recent first.</p>
              </div>
                  ...
                  <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-zinc-200 text-sm">
                      ...
                      <tbody className="divide-y divide-zinc-100">
                        {orders.map((order) => (
                          <tr key={`${order.orderId}-${order.productName}-${order.variantDetail}`}>
                            <td className="px-3 py-2">
                              <div className="flex items-center gap-2">
                                <span className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-[10px] font-bold ${getAvatarClass(order.customerName)}`}>
                            ...
                            <td className="px-3 py-2 text-right font-semibold text-zinc-800">{order.quantity}</td>
            ...
            <section className="mb-6 rounded-lg bg-white shadow-sm ring-1 ring-zinc-200">
              <div className="border-b border-zinc-200 px-6 py-4">
                ...
                <p className="mt-0.5 text-sm text-zinc-500">Click a product for the buyer breakdown.</p>
              </div>
              <div className="px-6 py-5">
            ...
            <section className="rounded-lg bg-white shadow-sm ring-1 ring-zinc-200">
              <div className="border-b border-zinc-200 px-6 py-4">
                <h2 className="text-xl font-semibold text-zinc-900">Repeat buyers</h2>
                <p className="mt-0.5 text-sm text-zinc-500">Buyers who've bought more than one product/variant in this date range.</p>
              </div>
              <div className="px-2 py-2 sm:px-4">
                {repeatBuyers.length === 0 ? (
                  <p className="px-4 py-6 text-sm text-zinc-500">No buyers with multiple products in this date range yet.</p>
                ) : (
                  <ul className="divide-y divide-zinc-100">
                    {repeatBuyers.map((buyer) => (
                      <li key={buyer.key} className="flex items-center justify-between gap-3 px-2 py-3">
                        <div className="flex items-center gap-3">
                          <span className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-xs font-bold ${getAvatarClass(buyer.name)}`}>
                        ...
                        <div className="flex items-center gap-2 text-sm">
                          <span className="rounded-full bg-zinc-100 px-2 py-1 text-xs font-medium text-zinc-700">
                            {buyer.productCount} products
                          </span>
                          <span className="rounded-full bg-red-50 px-2 py-1 text-xs font-medium text-red-700">
                            {buyer.totalQuantity} total
                          </span>
                        </div>
                      </li>
                    ))}
