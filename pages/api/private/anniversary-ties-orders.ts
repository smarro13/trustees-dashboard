...
// Who can view 90th Anniversary Ties sales analytics — mirrors
// canRoleViewAgendaHref in lib/roles.ts for '/agenda/anniversary-ties'.
...
type SquarespaceVariantOption = { optionName?: string; value?: string };
...
// promo code (e.g. a comped order) shows up only in the order's
...
// e.g. [{optionName:"Size",value:"Large"}] -> "Size: Large". Joins multiple
// options with " / " so a colour+size variant both show up. This is how
// "what they ordered" is captured even when Squarespace doesn't split a
// variant out into its own product name.
const getVariantDetail = (item: SquarespaceLineItem): string =>
  (item.variantOptions || [])
    .filter((opt) => opt.value)
    .map((opt) => (opt.optionName ? `${opt.optionName}: ${opt.value}` : opt.value))
    .join(' / ');
...
const MAX_PAGES = 20; // 20 * 50 orders = up to 1000 orders per request, plenty for a single campaign
  ...
  for (let page = 0; page < MAX_PAGES; page += 1) {
    ...
    const response = await fetch(url.toString(), {
      headers: {
        ...
        'User-Agent': 'Aldwinians Trustees Dashboard (90th anniversary ties analytics)',
      },
    });
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const daysParam = typeof req.query.days === 'string' ? parseInt(req.query.days, 10) : 365;
  const days = Number.isFinite(daysParam) && daysParam > 0 ? Math.min(daysParam, 730) : 365;
  ...
  // Lets the caller drop low-value add-on line items (e.g. a delivery
  // charge) that aren't actual tie sales but would otherwise show up as a
  // "product".
  ...
  try {
    ...
    // Tracks distinct products each customer has bought into — a "repeat
    // buyer" is someone who's bought more than one matching product (e.g.
    // more than one variant), not just someone with multiple orders for the
    // same one. Keyed by Squarespace's stable customerId (see
    // getCustomerKey), not display name.
    ...
    for (const order of orders) {
      ...
      for (const item of order.lineItems || []) {
        ...
        lineItemDetails.push({
          ...
          variantDetail: getVariantDetail(item),
          quantity: qty,
          unitPrice,
    ...
    return res.status(200).json({
      ...
      // Buyers who've bought more than one matching product (e.g. more than
      // one variant) in this date range.
      repeatBuyers: [...byCustomer.entries()]
        .map(([key, stats]) => ({ key, name: stats.name, productCount: stats.products.size, totalQuantity: stats.quantity }))
        .filter((r) => r.productCount >= 2)
        .sort((a, b) => b.productCount - a.productCount || b.totalQuantity - a.totalQuantity),
      discountUsage,
      orders: lineItemDetails,
      generatedAt: new Date().toISOString(),
    });
  } catch (err: any) {
    console.error('90th anniversary ties analytics error:', err);
